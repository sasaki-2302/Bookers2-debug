class ChatsController < ApplicationController
  ...
  def show
    ...
    rooms = current_user.user_rooms.pluck(:room_id)#ログイン中のユーザーの部屋情報を全て取得 pluckは指定したカラムのレコードを全て取得
    user_rooms = UserRoom.find_by(user_id: @user.id, room_id: rooms)#その中にチャットする相手とのルームがあるか確認

    unless user_rooms.nil?#ユーザールームがnilでないなら
  ...
  def create
    ...
    render :validater unless @chat.save #新規投稿されなければvalidater,されればcreateを探しに行く
  end
